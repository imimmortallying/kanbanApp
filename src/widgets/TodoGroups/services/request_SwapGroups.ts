...
interface request_SwapGroups {
    username: string;
    groups: [];
}

export const request_SwapGroups = createAsyncThunk <Number, request_SwapGroups, { rejectValue: string }>(
    'request_swapGroups', // что это?
    async (authData, thunkAPI) => {
        // console.log('auth: ', authData)
        try {
            const response = await axios.post('http://localhost:8000/swapGroups', authData);
            ...
            thunkAPI.dispatch(swapGroupsResponse(response.data))
